...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.user_data = {}  # Kullanıcı verilerini saklamak için bir boş sözlük tanımlayın
    ...
    def send_time(self):
        ...
        if self.current_user:
            ...
            self.veritabanina_ekle(veri, selected_date)

    def veritabanina_ekle(self, veri, selected_date):
        conn = sqlite3.connect("veritabani.db")
        cursor = conn.cursor()
        username = self.current_user

        cursor.execute('''INSERT INTO saatler (isim, tarih, saat, islem)
                          VALUES (?, ?, ?, ?)''',
                       (username, selected_date, veri['saat'], veri['islem']))
